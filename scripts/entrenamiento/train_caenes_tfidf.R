...
network_inputs <- pre_process(df, text_variable =  "glosa_dep_stemm", label =  "cod_final", type = "tfidf")
...
# Save the model
save_model_hdf5(model, "data/finales/modelos/modelo_red_tfidf")
#save_text_tokenizer(network_inputs$tokenizer, filename = "data/finales/modelos/tokenizer_stemm")

# Save the orginal labels
write_feather(network_inputs$keys, path = "data/finales/modelos/keys.feather")
...
model <- load_model_hdf5("data/finales/modelos/modelo_red_tfidf")
