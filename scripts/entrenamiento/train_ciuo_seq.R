...
network_inputs <- pre_process_ciuo(df, "glosa_dep_stemm", "cod_final")
...
# Save the model
save_model_hdf5(model, "data/finales/modelos/modelo_red_seq_ciuo")

# Save the tokenizer
save_text_tokenizer(network_inputs$tokenizer, filename = "data/finales/modelos/tokenizer_stemm_ciuo")


# Save keys to recover original labels
write_feather(network_inputs$keys, "data/finales/modelos/keys_ciuo.feather")
...
# model <- load_model_hdf5("data/finales/modelos/modelo_red_seq_ciuo")
...
#    bind_cols(df2 %>% select(glosa_dep, glosa_ciuo, glosa_dep_stemm, idrph, mes))
#  
