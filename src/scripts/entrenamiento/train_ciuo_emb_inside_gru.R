...
# Save the model
save_model_hdf5(model, "data/finales/modelos/modelo_red_emb_gru_ciuo")
...
# model <- load_model_hdf5("data/finales/modelos/modelo_red_emb_gru")
...
# predicho <- data.frame(predicho = predictions, cod_real = df2$cod_final) %>%
#   left_join(network_inputs$keys, by =  c("predicho" = "codigo_int")) 
# 
