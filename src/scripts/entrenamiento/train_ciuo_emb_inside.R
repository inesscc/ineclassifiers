...
network_inputs <- pre_process_ciuo(df, text_variable =  "glosa_dep", label =  "cod_final", type = "sequences")
...
# Save the model
save_model_hdf5(model, "data/finales/modelos/modelo_red_emb_simple_ciuo")
save_text_tokenizer(network_inputs$tokenizer, filename = "data/finales/modelos/tokenizer_ciuo")
...
model <- load_model_hdf5("data/finales/modelos/modelo_red_emb_simple_ciuo")
tokenizer <- load_text_tokenizer("data/finales/modelos/tokenizer_ciuo")
...
predicho <- data.frame(predicho = class_pred, cod_real = df2$cod_final) %>%
  ...
  bind_cols(df2 %>% select(glosa_dep, glosa_ciuo, idrph, mes))

mean(predicho$cod_real == predicho$cod_pred)
# 
# # Guardar
# write_feather(predicho, 'data/finales/predicciones/predict_emb_inside_red.feather')
# 
